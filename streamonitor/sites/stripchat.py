...
class StripChat(Bot):
    ...
    def getInitialData(cls):
        ...
        doppio_js_index = re.findall(
            '([0-9]+):"Doppio"', StripChat._main_js_data
        )[0]

        doppio_js_hash = re.findall(
            f'{doppio_js_index}:\\"([a-zA-Z0-9]{{20}})\\"',
            StripChat._main_js_data
        )[0]

        r = requests.get(
            f"{mmp_base}/chunk-Doppio-{doppio_js_hash}.js",
            headers=cls.headers,
            proxies=cls.get_proxies()
        )
